...
class MainActivity : AppCompatActivity()
{
    ...
    override fun onCreate(savedInstanceState: Bundle?)
    {
        ...
        val plusOneBtn = Button(this)
        plusOneBtn.text = "+1"

        val tvPO = TextView(this)
        tvPO.text = "pipppo"

        plusOneBtn.setOnClickListener {
            val txt = tvPO.getText().toString()
            val txtValue = txt.toIntOrNull()

            tvPO.text = txtValue?.inc()?.toString() ?: "0"
        }
        ...
        plusOneBtn.id = View.generateViewId()
        tvPO.id = View.generateViewId()
        ...
        myLayout.addView(plusOneBtn)
        myLayout.addView(tvPO)
        ...
        mySet.connect(plusOneBtn.id, ConstraintSet.LEFT, ConstraintSet.PARENT_ID, ConstraintSet.LEFT)
        mySet.connect(plusOneBtn.id, ConstraintSet.TOP, bu.id, ConstraintSet.BOTTOM)
        mySet.connect(tvPO.id, ConstraintSet.LEFT, plusOneBtn.id, ConstraintSet.RIGHT)
        mySet.connect(tvPO.id, ConstraintSet.BASELINE, plusOneBtn.id, ConstraintSet.BASELINE)
